...
shifts of string+$.We consider a prefix of len. 2^k 
of the cyclic, in the kth iteration.String of len. 
2^k->combination of 2 strings of len. 2^(k-1), whose
order we know, from previous iteration.Just radix 
sort on pair for next iteration.
Time :- O(nlog(n) + alphabet). Applications :-
Finding the smallest cyclic shift;Finding a substring
in a string; Comparing two substrings of a string;
Longest common prefix of two substrings; Number of 
different substrings. */
//returns permutation of indices in sorted order ***
...
// For comp. two substr. of len. l starting at i,j.
// k - 2^k > l/2. check the first 2^k part, if equal,
// check last 2^k part. c[k] is the c in kth iter 
//of S.A construction. 
int compare(int i, int j, int l, int k) {
	pll a = {c[k][i],c[k][(i+l-(1 << k))%n]};
	pll b = {c[k][j],c[k][(j+l-(1 << k))%n]};
	return a == b ? 0 : a < b ? -1 : 1; }
/*lcp[i]=len. of lcp of ith & (i+1)th suffix in the SA
1.Consider suffixes in decreasing order of length.
2.Let p = s[i....n]. It will be somewhere in the S.A.
We determine its lcp = k. 3.Then lcp of q=s[(i+1)..n] 
will be atleast k-1 coz 4.remove the first char of p 
and its successor in the S.A. These are suffixes with 
lcp k-1. 5.But note that these 2 may not be consecutive 
in S.A.But lcp of str. in b/w have to be also >= k-1.*/
